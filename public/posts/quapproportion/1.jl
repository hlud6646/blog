...
# ╔═╡ a51e35e0-e59b-11ee-23c0-eff1002790d9
begin
	using CairoMakie
	using Distributions
	using KernelDensity
...
# ╔═╡ a59376c8-12fa-4326-bb97-b48a6ebf1c7d
md"""
frontmatter
---
...
# ╔═╡ e2ac4746-2e0d-4f37-a330-06e2e71b70e4
md"""
...
If, for example, the parameter to estimate is a proportion and therefore bound to the interval $[0,1]$, it might be difficult to fit a Gaussian, where a significant amount of probabilty mass might lie in the tails and outside of $[0,1]$. To overcome this we can consider a transformed parameter which is supported on the whole real line, make our approximation, find whatever summary statistic we are interested in, and then transform it back to the original scale.
"""

# ╔═╡ 984d1fca-86ec-499b-93e6-aa199522298e
...
function coef_table!(ax, intervals, names; grouping = nothing)
	...
	rangebars!(ax, ypos, intervals, direction = :x, color = :black, whiskerwidth = 10)
...
# ╔═╡ dce1bb3d-95f1-4c78-9a37-c8ea68fc5caa
md"""
#### Model
We assume that the data follow a $Bernoulli(γ)$ distribution.
"""

# ╔═╡ 15e9e113-4cd0-436f-a848-36f23b8f4a12
begin
	Random.seed!(402)
...
# ╔═╡ f56f9b3e-87c4-44a0-b5f5-66244f4ff19a
md"""
#### 
and use the prior
...
# ╔═╡ 60c99f93-b991-4d0d-b865-bca618bd8da3
md"""
#### Analytic Posterior
Using this prior, the posterior denisty given $n$ trials and $s$ successes is

$$\gamma \sim Beta(1/2 + s, 1/2  + n - s)$$
"""

# ╔═╡ 54e5206a-d10d-4acd-afa2-047e4d56e45a
begin 
	y = sample_data()
	n = length(y)
	s = sum(y)
...
# ╔═╡ aa8a876f-ad9d-4a45-95f5-461cd9fb5b11
md"""
...
# ╔═╡ c996f8ff-08b8-42ed-b094-56b497c9cdda
begin
	γ̂ = mode(posterior)
...
# ╔═╡ 78869439-7815-4dc0-9382-d412f1833234
md"""
#### Reparameterising
...
# ╔═╡ 57b12f7a-1184-4eb5-9819-017b09b12585
begin
	σ(θ) = 1 / (1 + exp(-θ))
	logit(γ) = log(γ/(1 - γ))
...
# ╔═╡ eec6a7f0-f6a0-4577-9cf4-e070b91291e4
begin
...
# ╔═╡ 8bfc2e43-1922-4232-8064-88558c7fb522
fig1

# ╔═╡ d96f367d-7302-4a33-a6b7-f6548eb61b92
md"""
...
# ╔═╡ bd79d04f-0a80-4ee0-8e14-2b456f215819
begin
...
# ╔═╡ f1b6d2a2-4b43-4ed9-9b23-b6b08c5cefd1
fig2
...
manifest_format = "2.0"
project_hash = "d7cca03ab2e2db3feb9eb6abab05d8486404ff3e"
